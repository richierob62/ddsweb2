	...
	return (state = initial_state, action = {}) => {
		switch (action.type) {
			case BEGIN_x_CREATE: {
				...
				const new_copy = state.get('fields').reduce((acc, field, index) => {
			...
			case BEGIN_x_DUPLICATE: {
				...
				const current = state.get('list').find((item) => item.get('id') === id).toJS()
			...
			case CANCEL_x: {
				...
				switch (mode) {
					...
					case 'edit': {
						...
						return state
							.updateIn([ 'list' ], (list) => list.set(index, backup_copy))
							.set('mode', 'display')
					...
					case 'new': {
						...
						return state
							.updateIn([ 'list' ], (list) => list.delete(index))
							.set('mode', 'display')
							.set('selected_id', -1)
					}
			...
			case LOAD_x_LIST_COMPLETED: {
				...
				const selectedIdInNewList =
					selected_id === -1
						? false
						: action.payload.filter((entry) => entry.id === selected_id).length === 0 ? false : true

				const newList = Immutable.fromJS(action.payload)
				return selectedIdInNewList
					? state.set('list', newList).set('list_dirty', false).set('first_index', 0)
					: state.set('list', newList).set('selected_id', -1).set('list_dirty', false).set('first_index', 0)
			}

			case LOAD_x_REFERENCE_COMPLETED: {
				const newList = Immutable.fromJS(action.payload)
				return state.set('ref_list', newList).set('ref_list_dirty', false)
			}

			case SAVE_x_COMPLETED: {
				const mode = state.get('mode')

				if (mode === 'edit') return state.set('mode', 'display').set('backup_copy', undefined)
			...
			case PREVIOUS_x_SUBLIST: {
				const new_idx = state.get('first_index') - 5 < 0 ? 0 : state.get('first_index') - 5
			...
			case NEXT_x_SUBLIST: {
				...
				const new_idx =
					state.get('first_index') + 10 > list_size
			...
			case 'SORT_CHANGE_COMPLETED': {
				if (action.payload.reducer === state.get('reducer_name')) {
					...
					if (is_dirty) {
						return state
							.setIn([ 'current_sort', 'field_name' ], action.payload.field_name)
							.setIn([ 'current_sort', 'direction' ], action.payload.direction)
							.set('first_index', 0)
