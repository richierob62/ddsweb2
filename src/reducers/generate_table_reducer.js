    ...
    switch (action.type) {
      ...
      case LOAD_x_LIST_COMPLETED: {
        ...
        const selectedIdInNewList =
          selected_id === -1
            ? false
            : action.payload.filter(entry => entry.id === selected_id)
                .length === 0
              ? false
              : true
      ...
      case PREVIOUS_x_SUBLIST: {
        const new_idx =
          state.get('first_index') - 5 < 0 ? 0 : state.get('first_index') - 5
      ...
      case 'SORT_CHANGE_COMPLETED': {
        if (action.payload.reducer === table_name) {
          const is_dirty =
            action.payload.field_name !==
            state.getIn(['current_sort', 'field_name'])
